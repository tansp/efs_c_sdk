...
Efs_Posix_Handle Efs_Posix_Open(const char* file, int oflag, int mode)
{
	Efs_Posix_Handle fd = CreateFileA(
...
ssize_t Efs_Posix_Pread2(Efs_Posix_Handle fd, void* buf, size_t nbytes, Emu_Off_T offset)
{
	BOOL ret;
	DWORD nreaded = 0;
	DWORD tmpErrno = 0;
...
int Efs_Posix_Fstat2(Efs_Posix_Handle fd, Emu_FileInfo* buf)
{
...
int Efs_Posix_Close(Efs_Posix_Handle fd)
{
...
unsigned _int64 Efs_Posix_GetTimeOfDay(void)
{
	FILETIME tv;
	ULARGE_INTEGER uint;
